...
def initialize_model(model_name="resnet50", lr=0.001):
    ...
    # Modify the final layer to match the number of classes (2)
    num_features = model.fc.in_features
    model.fc = nn.Linear(num_features, 2)  # Two output classes: real (0) and fake (1)
...
def calculate_metrics(outputs, labels):
    ...
    preds = (pred_probs >= 0.5).astype(int)  # Binary predictions
    ...
    # Compute ROC curve for EER calculation
...
# Train and validate function update for logging metrics
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, epochs=5, device='cuda', log_path="training_log.txt", output_dir="models"):
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                for images, labels in tqdm(val_loader, desc=f"Epoch {epoch+1}/{epochs} - Validation"):
                    ...
                    pred_probs = torch.sigmoid(outputs).cpu().numpy()  # Apply sigmoid for binary classification
                    preds = (pred_probs >= 0.5).astype(int)

                    val_probs.extend(pred_probs)
            ...
            metrics = calculate_metrics(torch.tensor(val_probs), np.array(val_labels))
            val_metrics.append(metrics)

            # Log metrics
            log_msg = (f"Epoch {epoch+1}/{epochs}, Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, "
                       f"Val Accuracy: {metrics['overall_accuracy']:.4f}, Precision: {metrics['precision']:.4f}, "
                       f"Recall: {metrics['recall']:.4f}, AUC: {metrics['auc']:.4f}, EER: {metrics['eer']:.4f}\n")
...
# Test model update for 2 classes
def test_model(model, test_loader, device='cuda'):
    ...
    with torch.no_grad():
        for images, labels in tqdm(test_loader, desc="Testing"):
            ...
            test_probs.extend(pred_probs)
            test_preds.extend(preds)
            test_labels.extend(labels.cpu().numpy())

    # Calculate metrics
    metrics = calculate_metrics(torch.tensor(test_probs), np.array(test_labels))
    ...
    print(f"Test Accuracy: {test_accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, AUC: {auc:.4f}, EER: {eer:.4f}")
