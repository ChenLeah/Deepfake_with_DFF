...
# Argument parsing
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune EfficientNet/ResNet model")
    parser.add_argument("--model_name", type=str, default="efficientnet_b0", choices=["resnet50", "efficientnet_b0", "efficientnet_b1", "efficientnet_b2"], help="Specify the model variant")
    ...
    parser.add_argument("--output_name", type=str, default="model_finetuned.pth", help="Name of the saved model file")
...
def prepare_data(root_dir, batch_size=32, val_split=0.15, test_split=0.15, input_size=224):
    dataset = DFFDataset(root_dir=root_dir, transform=get_transform(input_size=input_size))
...
def initialize_model(model_name="efficientnet_b0", lr=0.001):
...
def main():
    ...
    # Adjust input size based on model
    input_size = 224  # Default for EfficientNetB0 and ResNet
    if args.model_name == "efficientnet_b1":
        input_size = 240
    elif args.model_name == "efficientnet_b2":
        input_size = 260

    train_loader, val_loader, test_loader = prepare_data(args.root_dir, args.batch_size, input_size=input_size)
    ...
    train_losses, val_losses = train_and_validate(model, train_loader, val_loader, criterion, optimizer, args.epochs, 'cuda', log_path)
